  ...
  const handleResetDiscipline = () => persistDiscipline(resetDisciplineState(discipline));
    ...
    <Card variant="primary" className="p-3 relative">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-th-text">Execution Gate</h3>

        <div className="ml-auto flex items-center gap-2">
          <span className="text-[11px] px-2 py-1 rounded-full bg-th-hl/5 border border-th-border">
            {execState}
          </span>

          {isArmedOrMore && (
            <span className="text-[11px] px-2 py-1 rounded-full bg-th-hl/5 border border-th-border inline-flex items-center gap-1">
      ...
      <div className="mt-3 flex flex-col gap-2">
        <label className="flex items-center gap-2 text-xs text-th-text">
          <input
            type="checkbox"
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <button
          ...
          className={`h-9 px-4 rounded-full text-sm font-semibold transition border ${
            isEntered
              ...
              : !isArmed
              ? "bg-th-hl/5 text-th-text-muted border-th-border cursor-not-allowed"
              : "bg-gradient-to-r from-th-accent to-th-cta text-[#020617] hover:brightness-110 shadow-[0_0_18px_rgba(0,255,163,.12)] border-th-border"
          }`}
        ...
        <button
          ...
          className="ml-2 text-th-text-dim hover:text-th-text"
        >
      ...
      {showHistory && (
        <div className="fixed inset-0 bg-black/60 z-50">
          <div className="absolute right-0 top-0 h-full w-[340px] bg-th-base border-l border-th-border p-4">
            <h4 className="text-sm font-semibold text-th-text mb-2">
            ...
            {DEV_MODE && (
              <div className="flex flex-col gap-2 mb-3">
                <button
                  ...
                  className="w-full h-9 rounded-lg bg-th-hl/5 border border-th-border text-sm text-th-text hover:bg-th-hl/10 inline-flex items-center justify-center gap-2"
            ...
            <div className="space-y-2">
              {(discipline.violations || []).length === 0 && (
                <div className="text-xs text-th-text-muted">No violations</div>
              )}

              {(discipline.violations || []).map((v, i) => (
                <div
                  key={i}
                  className="p-2 rounded-lg bg-th-hl/5 border border-th-border"
                >
                  <div className="text-xs text-th-text">{violationLabel(v.type)}</div>
            ...
            <button
              className="mt-4 w-full text-sm px-3 py-2 rounded-lg bg-th-hl/5 border border-th-border hover:bg-th-hl/10"
