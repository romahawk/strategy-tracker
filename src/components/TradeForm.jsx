...
export default function TradeForm({
  ...
  useEffect(() => {
    if (editingTrade) {
      const hasEpoch = typeof editingTrade.openedAtMs === "number";
      const patched = hasEpoch
        ? editingTrade
        : { 
            ...editingTrade, 
            openedAtMs: buildOpenedAtMs(editingTrade.date, editingTrade.time) 
          };
      setForm({ ...patched });
    } else {
      setForm((prev) => ({
  ...
  const handleSubmit = (e) => {
    ...
    const openedAtMs = buildOpenedAtMs(form.date, form.time);
